...
\item Read all the HTML tables available in the above link and store the result in an object called \texttt{tables}. Note that you may have ti use the function \texttt{getURL()} as we talked in class. 
