...
\textbf{NOTE: } These plots are not showing up correctly in \texttt{Knitr}, but work fine in RStudio. I tried clearing out all variables and closing out RStudio, but I still can't get them to match.
